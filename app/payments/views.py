...
class PaymentView(viewsets.ModelViewSet):
    ...
    def list(self, request):
        try:
            resp = Payment.get_all(request)
            serializer = self.serializer_class(resp, many=True)
            return Resonse({"data": serializer.data}, status=status.HTTP_200_OK)
    ...
    def create(self, request):
        try:
            user_payed = User.objects.get(email=request.data.get("email")) 
            campaing_payed = Campaing.objects.get(id=request.data.get("campaing_id"))
            reward_payed = Reward.objects.get(id=reques.data.get("reward_id"))
            resp = Payment.created(request, user_payed, campaing_payed, reward_payed)
            return Response(
                    {"data": resp, "msg": "payment send and created"},
    ...
    def update(self, request, pk):
        try:
            campaing_payed = Campaing.objects.get(id=pk)
            resp = Payment.updated(request, campaing_payed)
            return Response(
                    {"data": resp, "msg": "payment recived"},
    ...
    #
    #         laData = {"tcParametros": tcParametros, "tcCommerceID": tcCommerceID}
    #
