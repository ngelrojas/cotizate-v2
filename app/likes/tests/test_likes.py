...
class LikeTests(TestCase):
    def setUp(self):
        ...
        self.like = create_like(**data_like)

    def test_create_update_like(self):
        """test create/update likes"""
        payload = {
                'user': self.user,
                'campaing_id': self.campaing.id,
                'liked': True
                }
        res = self.client.put(reverse('like:detail-like', 
                kwargs={'pk': self.like.id}),
                data=payload
                )
